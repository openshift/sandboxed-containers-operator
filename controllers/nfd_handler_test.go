...
func TestOscNodeTypeRule_FourRules(t *testing.T) {
	t.Parallel()
	r := newTestReconciler(fake.NewClientBuilder().Build())
	rule := r.oscNodeTypeRule()
...
func TestOscNodeTypeRule_BareMetalRule(t *testing.T) {
	...
	if labels["kataconfiguration.openshift.io/node-type"] != "bare-metal" {
		t.Errorf("node-type label: got %q, want bare-metal", labels["kataconfiguration.openshift.io/node-type"])
	}
	if labels["kataconfiguration.openshift.io/kata-capable"] != "true" {
		t.Errorf("kata-capable label: got %q, want true", labels["kataconfiguration.openshift.io/kata-capable"])
	}
...
func TestOscNodeTypeRule_NestedVirtIntelRule(t *testing.T) {
	t.Parallel()
	r := newTestReconciler(fake.NewClientBuilder().Build())
	rule := r.oscNodeTypeRule()

	rules, _, _ := unstructured.NestedSlice(rule.Object, "spec", "rules")
	nested := rules[2].(map[string]interface{})

	if nested["name"] != "osc-nested-virt-intel" {
		t.Errorf("nested intel rule name: got %q", nested["name"])
	}
	labels := nested["labels"].(map[string]interface{})
	if labels["kataconfiguration.openshift.io/kata-capable"] != "true" {
		t.Errorf("kata-capable label: got %q, want true", labels["kataconfiguration.openshift.io/kata-capable"])
	}
	if labels["kataconfiguration.openshift.io/nested-virt-capable"] != "true" {
		t.Errorf("nested-virt-capable label: got %q, want true", labels["kataconfiguration.openshift.io/nested-virt-capable"])
	}
	features := nested["matchFeatures"].([]interface{})[0].(map[string]interface{})
	exprs := features["matchExpressions"].(map[string]interface{})
	hypervisor := exprs["HYPERVISOR"].(map[string]interface{})
	if hypervisor["op"] != "Exists" {
		t.Errorf("HYPERVISOR op: got %q, want Exists", hypervisor["op"])
	}
	vmx := exprs["VMX"].(map[string]interface{})
	if vmx["op"] != "Exists" {
		t.Errorf("VMX op: got %q, want Exists", vmx["op"])
	}
}

func TestOscNodeTypeRule_NestedVirtAmdRule(t *testing.T) {
	t.Parallel()
	r := newTestReconciler(fake.NewClientBuilder().Build())
	rule := r.oscNodeTypeRule()

	rules, _, _ := unstructured.NestedSlice(rule.Object, "spec", "rules")
	nested := rules[3].(map[string]interface{})

	if nested["name"] != "osc-nested-virt-amd" {
		t.Errorf("nested amd rule name: got %q", nested["name"])
	}
	labels := nested["labels"].(map[string]interface{})
	if labels["kataconfiguration.openshift.io/kata-capable"] != "true" {
		t.Errorf("kata-capable label: got %q, want true", labels["kataconfiguration.openshift.io/kata-capable"])
	...
	svm := exprs["SVM"].(map[string]interface{})
	if svm["op"] != "Exists" {
		t.Errorf("SVM op: got %q, want Exists", svm["op"])
	}
}
...
func TestEnsureNodeFeatureRule_UpdatesWhenExists(t *testing.T) {
	...
	if len(rules) != 4 {
		t.Errorf("expected 4 rules after update, got %d", len(rules))
	}
}
