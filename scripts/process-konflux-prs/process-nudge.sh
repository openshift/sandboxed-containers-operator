...
while IFS= read -r group_key; do
    if [ -z "$group_key" ]; then
        while IFS= read -r pr; do
            skipped=$(echo "$skipped" | jq --argjson p "$pr" \
                '. + [$p + {"_skip_reason":"no components detected"}]')
        done < <(echo "$working" | jq -c '[.[] | select((.components | length) == 0)] | .[]')
        continue
    fi
    ...
    if [ "$count" -eq 1 ]; then
        ...
        while IFS= read -r pr; do
            ...
            # PRs with ok-to-test but checks not passing
            if [ "$build_ok" != "true" ] || [ "$pending" -gt 0 ] || [ "$other_n" -gt 0 ]; then
