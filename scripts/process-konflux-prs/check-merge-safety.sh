...
# Returns: "success", "failure", etc. from the check-run conclusion,
#          "unknown" when no matching check-run exists in the last 20 commits,
#          or "error" when the GitHub API query itself fails.
gh_last_push_conclusion() {
    ...
    shas=$(gh api "repos/${gh_repo}/commits?sha=${branch}&per_page=20" \
        --jq '.[].sha' 2>/dev/null) || { echo "error"; return; }

    while IFS= read -r sha; do
        local raw conclusion
        raw=$(gh api "repos/${gh_repo}/commits/${sha}/check-runs" \
            2>/dev/null) || { echo "error"; return; }
        conclusion=$(echo "$raw" | jq -r --arg n "$check_name" \
...
for component in "${COMPONENTS[@]}"; do
    ...
    if [[ -n "$GH_REPO" && -n "$GH_BRANCH" ]]; then
        ...
        if [[ "$conclusion" == "error" ]]; then
            entry=$(jq -n \
                --arg component "$component" \
                --arg pipeline "GitHub API" \
                --arg reason "Failed to query on-push pipeline status for branch ${GH_BRANCH}" \
                '{component: $component, pipeline: $pipeline, reason: $reason}')
            blocking=$(echo "$blocking" | jq --argjson e "$entry" '. + [$e]')
        elif [[ "$conclusion" != "success" && "$conclusion" != "unknown" ]]; then
